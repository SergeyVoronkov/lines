	...
	*/
	add:function(o,event,callback)
	{
		...
		if(!object.__events[event])
		{
			...
			{
				var arg = arguments;
				try{if(e==undefined || (Event && e instanceof Event)) arg = [jsFW.event.fix(e || window.event)];}catch(e){}
...
(function()
{
	...
	jsFW.displayBlock = function(o)
	{
		...
		{
			tagSelect = [];
			jsFW.element.getTag('select',function(el)
			{
				tagSelect.push({el:el,old:el.style.visibility || ''});
	...
	jsFW.displayUnblock = function()
	{
		...
		if(tagSelect)
		{
			...
			{
				tagSelect[i].el.style.visibility = tagSelect[i].old || '';
			}
			tagSelect = false;
		}
	};
...
jsFW.aspects.before(Array.prototype,'reduce|map|filter|each|sort',function(arg)
{
